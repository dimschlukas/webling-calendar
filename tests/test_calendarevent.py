...
END = BEGIN + DURATION
IS_ALL_DAY = False
EVENT = Event(title=TITLE, description=DESCRIPTION, place=PLACE,
              begin=BEGIN, end=END, duration=DURATION, is_all_day=IS_ALL_DAY)
...
def test_get_data():
    ...
    assert data == {"properties": EVENT.get_dict(),
                    "parents": [1234]}
